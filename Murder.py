...
def get_average_sentence_length(text):
    ...
    new_sentence_2 = new_sentence.replace("!", ".")
    sentences_in_text = new_sentence_2.split(".")
    sentence_lengths = []
    for i in sentences_in_text:
        for x in i.split():
            sentence_lengths.append(x)
            count = float(len(sentence_lengths)/len(sentences_in_text))
    return count
...
def percent_difference(value1, value2):
    formula = (abs(value1 - value2)) / ((value1 + value2) / 2)
    return formula
...
def find_text_similarity(arg1, arg2):
    sentence_length_difference = percent_difference(get_average_sentence_length(arg1), get_average_sentence_length(arg2))
    sentence_length_similarity = abs(1 - sentence_length_difference)
    word_count_similarity = frequency_comparison(build_frequency_table(prepare_text(arg1)), build_frequency_table(prepare_text(arg2)))
    ngram_similarity = frequency_comparison(build_frequency_table(ngram_creator(prepare_text(arg1))), build_frequency_table(ngram_creator(prepare_text(arg2))))
    similarities = 0
    similarities += sentence_length_similarity
    similarities += word_count_similarity
    similarities += ngram_similarity
...
print("Lily" + " " + str(find_text_similarity(murder_note, lily_trebuchet_intro)))
print("Gregg" + " " + str(find_text_similarity(murder_note, gregg_t_fishy_intro)))
print("Myrtle" + " " + str(find_text_similarity(murder_note, Myrtle_beech_intro)))
